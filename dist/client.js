...
class ModbusClient {
  ...
  async setSlaveId(newSlaveId) {
    if (!Number.isInteger(newSlaveId) || newSlaveId < 0 || newSlaveId > 255) {
      throw new import_errors.ModbusInvalidAddressError(newSlaveId);
    }
    const old = this.slaveId;
    this.slaveId = newSlaveId;
    this._setAutoLoggerContext();
    logger.info(`Slave ID changed ${old} -> ${newSlaveId}`, {
        ...
        try {
          ...
          if (!(err instanceof import_errors.ModbusExceptionError)) {
            if (transport.notifyDeviceDisconnected) {
              ...
              if (err instanceof import_errors.ModbusTimeoutError) errorType = import_modbus_types.ConnectionErrorType.Timeout;
              else if (err instanceof import_errors.ModbusCRCError) errorType = import_modbus_types.ConnectionErrorType.CRCError;
