 ...
 * @param {Uint8Array} pdu - Ответ устройства (минимум 1 байт)
...
function parseSetControllerTimeResponse(pdu) {
    ...
    if (pdu.length < RESPONSE_MIN_SIZE || pdu[0] !== FUNCTION_CODE) {
        ...
        throw new Error(`Invalid response: expected min ${RESPONSE_MIN_SIZE} byte(s) with FC=0x${FUNCTION_CODE.toString(16)}, got ${pdu.length} byte(s) with FC=0x${receivedCode}`);
    }
...
module.exports = {
    ...
    // Экспорт констант для тестирования
    _TEST_CONSTANTS: { FUNCTION_CODE, REQUEST_SIZE }
};