...
class SlaveEmulator {
  ...
  handleRequest(buffer) {
    try {
      ...
      this.log.error('Error processing Modbus request', {
        error: error.message,
        stack: error.stack,
      });
    ...
    try {
      ...
      switch (functionCode) {
        ...
        case FUNCTION_CODES.WRITE_MULTIPLE_REGISTERS:
          ...
          break;
        default:
          this.log.warn('Unsupported function code', {
            functionCode: `0x${functionCode.toString(16)}`,
          });
          throw new ModbusExceptionError(functionCode, 1);
      }
  ...
  _createSuccessResponse(slaveAddr, functionCode, responseData) {
    ...
    respBuf[respBuf.length - 2] = crc & 255;
